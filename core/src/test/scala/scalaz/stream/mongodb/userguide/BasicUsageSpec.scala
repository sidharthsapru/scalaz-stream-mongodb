...
the collection), or `WriteResult` (when performing save, insert or update). Additional types depend on each operation on collection.
...
Query based updates are equivalents of mongoDB's _findAndModify_, _findAndRemove_ and _update_ commands.
...
${ snippet { query("name" -> "yoda") and remove } }
...
${ snippet { query("name" -> "yoda") and updateOne("ship" := None) } }
${ snippet { query("name" -> "yoda") and updateOne("ship" := None).returnNew(true) } }
${ snippet { query("name" -> "yoda").sort("last" Ascending) and updateOne(document) } }
...
${ snippet { query("name" -> "yoda") and removeOne } }
${ snippet { query("name" -> "yoda").sort("last" Ascending) and removeOne } }
